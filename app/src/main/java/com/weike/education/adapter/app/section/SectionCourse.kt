    ...
    override fun convert(holder: ViewHolder?, mBean: SelectionBean.Data.Course?, position: Int) {
        holder?.apply {
            mBean?.let {
                ...
                setText(R.id.title, mBean.courseTitle)
                setText(R.id.courseSaleTime_Num, "开课时间：${mBean.courseTime} ${mBean.lessonNum.toInt()}课时")
                setText(R.id.saleNum, if (mBean.courseSaleNum == 0) "" else "已有${mBean.courseSaleNum}人购买")

                if (mBean.courseSalePrice.toInt() == 0) {
                    setText(R.id.price, "免费").setVisible(R.id.yuan, false)
                } else {
                    setText(R.id.price, "${mBean.courseSalePrice}")
                }
